...
cp $DIR/../temp/* $DIR/../conf

#export USE_SUBJECT_CREDS="-Djavax.security.auth.useSubjectCredsOnly=false"

echo java $@ $JAVA_OPTS -XX:MaxPermSize=384m $SEARCH_PATH -cp "$CP" -Djava.library.path=$LD_LIBRARY_PATH org.trustedanalytics.atk.moduleloader.Module rest-server org.trustedanalytics.atk.rest.RestServerApplication
java $@ $JAVA_OPTS -XX:MaxPermSize=384m $SEARCH_PATH -cp "$CP" -Djava.library.path=$LD_LIBRARY_PATH org.trustedanalytics.atk.moduleloader.Module rest-server org.trustedanalytics.atk.rest.RestServerApplication
